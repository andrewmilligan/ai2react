...
function render(settings, customBlocks) {
  ...
  forEachUsableArtboard(function(activeArtboard, abIndex) {
    ...
    var imgSrcs;
    var imgName, imgVar;
    var artboardContent = {html: '', css: '', js: '', imgs: []};
    var i;
    ...
    if (abSettings.image_only || settings.render_text_as == 'image') {
      ...
      textData = react_convertTextFrames(textFrames, activeArtboard);
    }
    progressBar.step();
    ...
    if (isTrue(settings.write_image_files)) {
      ...
      imgSrcs = imageData.html.match(/src="(.*)"/g);
      for (i = 0; i < imgSrcs.length; i++) {
        imgName = imgSrcs[i].match(/src="(.*)"/)[1];
        imgVar = 'img' + imgId;
        artboardContent.imgs.push([imgVar, imgName]);
        imageData.html = imageData.html.replace(new RegExp(imgName, 'g'), '${' + imgVar + '}');
        imgId += 1;
      }
    } else {
      imageData = {html: ''};
  ...
  forEach(fileContentArr, function(fileContent) {
    ...
    react_generateOutputHtml(fileContent, fileContent.name, settings);
  });
...
function react_debugScript(data, name) {
  const json = JSON.stringify(data, null, 2)
  return '<script type="application/json" data-name="'+name+'">'+json+'</script>\n'
}

function react_convertTextFrames(textFrames, ab) {
  var frameData = map(textFrames, function(frame) {
    return {
      paragraphs: importTextFrameParagraphs(frame)
    };
  ...
  var divData = map(frameData, function(obj, i) {
    var frame = textFrames[i];
    var divId = frame.name ? makeKeyword(frame.name) : idPrefix  + (i + 1);
    var positionCss = getTextFrameCss(frame, abBox, obj.paragraphs);
    return {
...
function assignArtboardContentToFile(name, abData, outputArr) {
  ...
  if (!obj) {
    obj = {name: name, html: '', js: '', css: '', imgs: []};
    outputArr.push(obj);
  }
  obj.html += abData.html;
  obj.js += abData.js;
  obj.css += abData.css;
  obj.imgs = obj.imgs.concat(abData.imgs || []);
}
...
function react_generateOutputHtml(content, pageName, settings) {
  var linkSrc = settings.clickable_link || '';
  var containerId = nameSpace + pageName + '-box';
  var textForFile, componentName;
  var htmlFileDestinationFolder, htmlFileDestination;
  var fileText, imgImportText;

  progressBar.setTitle('Writing HTML output...');

  componentName = doc.name.replace(/\.ai$/, '').replace(/\s/g, '_')

  imgImportText = map(content.imgs, function(i) {
    return 'import ' + i[0] + ' from \'./' + i[1] + '\''
  }).join('\r')
  textForFile = [
    '/* eslint-disable */',
    '/* Generated by ai2html v' + scriptVersion + ' - ' + getDateTimeStamp() + ' */',
    '/* ai file: ' + doc.name + ' */',
    '',
    'import { makeAi2React, resolveImages } from \'ai2react\'',
    imgImportText,
    '',
    'const styles = resolveImages`',
    generatePageCss(containerId, settings) + content.css,
    '`',
    '',
    'const html = props => resolveImages`',
    '<div id="' + containerId + '" class="ai2html">',
    linkSrc && ('\t<a class="' + nameSpace + 'ai2htmlLink" href="' + linkSrc + '">'),
    content.html,
    linkSrc && ('\t</a>'),
    '</div>',
    '`',
    ...
    '',
    '/* End ai2html' + ' - ' + getDateTimeStamp() + ' */',
  ].join('\r').replace(/\t/g, '  ')

  // textForFile = applyTemplate(textForFile, settings); // TODO?

  htmlFileDestinationFolder = docPath + settings.html_output_path;
  checkForOutputFolder(htmlFileDestinationFolder, 'html_output_path');
  htmlFileDestination = htmlFileDestinationFolder + pageName + '.js';

  if (settings.output == 'one-file' && settings.project_type == 'ai2html') {
    htmlFileDestination = htmlFileDestinationFolder + 'index' + '.js';
  }
